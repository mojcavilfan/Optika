...
se danes uporablja za celo vrsto raziskav, med drugim za neposredno opazovanje 
...
\caption{Postavitev Michelsonovega interferometra. Vpadno svetlobo z polprepustnim
zrcalom $PZ$ razdelimo na dve delni valovanji. Prvo se odbije od zrcala $Z_1$ in vrne
po isti poti, drugo pa od zrcala $Z_2$, ki se prav tako vrne po isti poti, zato se delni
valovanji na polprepustnem zrcalu ponovno združita. Interferenco delnih valovanj, ki je odvisna
od premika zrcala $x$, opazujemo na detektorju $D$. Pogosto v interferometer dodamo kompenzacijsko
ploščico $K$, da sta optični poti obeh žarkov $l_1$ in $l_2$ v izhodiščni legi povsem enaki.
}
\label{fig:06_Michelson}
\vglue-4truemm
\end{figure}

Gostoto svetlobnega toka na detektorju lahko izračunamo kot vsoto 
dveh valovanj z enako amplitudo (enačba~\ref{eq:06_09}):
\beq
j = 4j_0 \cos^2(\Delta \phi/2),
\label{eq:06_24}
\eeq
pri čemer je $\Delta \phi = k_0(l_1-l_2) = 2 k_0 x$. Ojačitve na detektorju dosežemo pri:
\beq
k_0 x = N \pi
\eeq
oziroma:
...
\eeq
Vrhovi se torej pojavljajo pri premiku zrcala za $\lambda/2$. 



Kompenzator dodamo, če kratka koherenčna razdalja, da se izniči razmik.
Če v eksperimentu uporabimo nekolimiran snop, se na opazovalnem zaslonu oziroma
detektorju pojavijo kolobarji, ki nastajajo oziroma izginjajo proti sredini, ko
pomikamo zrcalo M2. To se zgodi, ker imajo žarki, ki potujejo pod različnimi
koti, različno dolge poti. 
SLIKA: kako nastanejo krogi. 

... prakitkum:  med katerimi je
najbolj znan Michelson-Morleyev eksperiment, s katerim so ovrgli hipotezo o obstoju
vse-obdajajočega etra [2]. Danes se Michelsonovi interferometri uporabljajo za pre-
cizno merjenje dolžin in lomnih količnikov, z njimi kontroliramo majhne napake na ra-
zličnih optičnih komponentah, uporabljamo ga v visoko ločljivi infra-rdeči spektroskopiji
(Fourierova spektroskopija), itd. Prednosti? Slabosti?

Točkasti izvor svetlobe lahko pred vpadom na polprepustno zrcalo kolimiramo
z lečo. Na tak način izvedemo Twyman-Greenov in Mach-Zehnderjev interferometer.
Prvi je uporaben za testiranje homogenosti in ravnosti prozornih objektov. Prednost
Mach-Zenhderjevega sistema je, da gre svetloba čez vsako vejo interferometra samo enkrat.
Interferenco z delitvijo svetlobe dobimo tudi na tankih plasteh, pri nastanku
Newtonovih kolobarjev med lečo in podlago...
SLIKA TWYGreen & MACHZEHNDER.

Zanimiv je Sagnacov interferometer, ki se uporablja za laserske giroskope. En del svetlobe
potuje v smeri urinega kazalca, drugi del pa v nasprotni smeri. Če interferometer miruje, 
med njima dobimo konstruktivno interferenco. Če se interferometer vrti v ravnini žarkov,
pa se pojaivi fazni zamik in z njim zmanjševanje signala na detektorju. 
...
je:
\beq
\Delta t_1 = \frac{2\pi R- \Delta L}{c_0},
\eeq
pri čemer je $\Delta L - \Omega R \Delta t_1 = R \Delta \phi$. Vstavimo v gornjo enačbo in dobimo:
\beq
\Delta t_1 = \frac{2\pi R}{c_0+\Omega R}.
\eeq
Čas, ki ga potrebuje drugi žarek, da pride od izvora do detektorja, je podobno:
\beq
\Delta t_2 = \frac{2 \pi R}{c_0 -\Omega R}.
\eeq
Med obema signaloma nastane fazni zamik:
\beq
\Delta \phi - \omega (\Delta t_2 - \Delta t-1) = \omega \left(\frac{2\pi R}{c_0-\Omega R} - 
\frac{2 \pi R}{c_0 + \Omega R}  \right)\!\!.
\eeq
Sledi:
\beq
\Delta \phi = \frac{4 \pi^2 R^2 \omega \Omega}{c_0^2 - \Omega^2R^2} \approx 
