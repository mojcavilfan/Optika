...
\beq
E(0,0, z_0) = \frac{i}{\lambda} \frac{\tilde{E}_0 e^{ikz_0'+ikz_0}}{z_0'z_0} 
...
\end{remark}
\vglue4truemm
Fresnelovo uklonsko sliko (enačba~\ref{eq:05_97}) na pravokotni odprtini s Fresnelovima
integraloma zapišemo kot:
\beq
...
znatno razlikuje. Črtkana črta označuje velikost odprtine. Desno so simulirane uklonske
slike na režah.}
\label{fig:05_FrReza}
\end{figure}
\vglue5truemm
...
Poglejmo za primer Fresnelov uklon na ostrem robu. Intenziteta je sorazmerna vsoti 
kvadratov razlik integralov v končni in začetni točki (enačba~\ref{eq:05_103}). 
Začetna točka je v $(-1/2, -1/2)$ pri $u \to -\infty$, končna pa pri $u_2$.
Z spreminjajočim $u_2$ se končna točka daljice premika po spirali, zato njena 
dolžina, ki opisuje intenziteto valovanja, nekaj časa narašča, nato pojema in 
ponovno narašča (zelena črta) ... Spreminjanje dolžine daljice 
opiše uklonsko sliko v odprtini. Uklon v senci dobimo pri negativnih vrednostih 
parametra $u_2$, pri katerih se dolžina hitro približuje ničli (modra črta). 
\end{remark}
\end{example}
