...
\label{eq:05_83}
\eeq
Odvisnost gostote svetlobnega toka $j$ od polmera odprtine
...
\eeq
Kolobar z notranjim polmerom $a_1 = \sqrt{\lambda L}$ in zunanjim polmerom 
$a_2 = \sqrt{2\lambda L}$ imenujemo druga Fresnelova cona. 
...
polmeri kolobarjev pa so vedno manjše. 
...
se tako izmenično pojavljajo ojačitve in oslabitve. 
\end{remark}
...
Prispevki sekundarnih polj se seštevajo za $\Delta \phi < \pi$, nato se začnejo prispevki odštevati in 
polje zmanjševati (sredina). Fresnelova conska plošča s conami 1, 2, 5 in 7 (desno).}
\label{fig:05_FresCona3}
\end{figure}
Dokler je razlika faz manjša od $\pi$, se prispevki sekundarnih krogelnih valov iz odprtine
seštevajo (slika~\ref{fig:05_FresCona3}). 
Največja intenziteta svetlobe je tako dosežena pri $\Delta \phi = \pi$ in velja:
\beq
...
se pojavijo prispevki v protifazi. Prispevki iz prve cone in iz dodanega kolobarja se med seboj 
odštevajo, zato se intenziteta uklonjene svetlobe zmanjšuje. Ko fazni zamik doseže $2\pi$ (kar se zgodi
ravno pri $a_2$), se prispevki spet seštevajo in intenziteta narašča. 

Ker Fresnelova uklonska plošča s polmerom $a_1$ intenziteto svetlobe v 
točki $P_0$ poveča glede na primer brez plošče, pomeni, da vpadno svetlobo zbere in 
torej deluje kot zbiralna leča.  
...
lihi ali samo sodi, se prispevki vseh obstoječih con seštevajo (slika~\ref{fig:05_FresCona3}). 
Gostota svetlobnega toka v točki $P_0$ je pri uporabi conske plošče tako dosti večja, kot bi bila 
v primeru, če bi izvor svetlobe točko osvetljeval neposredno. Izračunajmo goriščno razdaljo 
take zbiralne leče. 

Goriščno razdaljo uklonske leče izračunamo iz pogoja, da nastane slika 
žarkov, ki na lečo vpadajo iz neskončnosti ($z_0'\to \infty$), pri $z_0 = f$:
\beq
\frac{1}{L} = \frac{1}{z_0'}+ \frac{1}{z_0}= \frac{1}{z_0} = \frac{1}{f},
\label{eq:05_91}
\eeq
od koder z uporabo enačbe~(\ref{eq:05_84a}) sledi:
\beq
f = \frac{a_1^2}{\lambda}.
\label{eq:05_92}
\eeq
