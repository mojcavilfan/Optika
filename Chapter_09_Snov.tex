...
to je hitrost premikanja ploskev konstantne faze (valovnih front), in
...
frekvence vpadne svetlobe v preprostem Lorentzevem modelu. Osenčen del na levi 
...
\label{eq:09_28}
\eeq
Namesto krožne frekvence lahko vstavimo valovno dolžino svetlobe in dobimo:
\beq
...
\begin{example}{\bf Kromatična aberacija leče.}
...
\centering
\def\svgwidth{140truemm} 
\input{slike/09_izolator.pdf_tex}
...
Zapišemo Newtonov zakon za gibanje kroglice,
...
nihanja $\omega^2 = k/m$, $m$ je masa elektrona, jakost električnega polja je $\mathbf{E} = (E_x, E_y, 0)$,
...
\beq
\ddot{z} -i \omega_{c}\dot{z} + \omega_0^2 z = -\frac{e_0}{m}E_L,
\label{eq:09_63}
\eeq
pri čemer smo vpeljali ciklotronsko frekvenco $\omega_{c} = e_0B_0/m$.
Drugič zapisani enačbi~(\ref{eq:09_52a} in \ref{eq:09_52b}) 
odštejmo in uvedemo novo spremenljivko $x-iy=w$ ter desno krožno polarizirano valovanje $E_D = E_x-iE_y$.
Dobimo:
\beq
\ddot{w} +i \omega_{c}\dot{w} + \omega_0^2 w = -\frac{e_0}{m}E_D.
\label{eq:09_64}
\eeq
...
\beq
z_0 = \frac{-e_0 E_L/m}{\omega_0^2 - \omega^2 - \omega_{c}\omega} \qquad \mathrm{in} \qquad 
w_0 = \frac{-e_0 E_D/m}{\omega_0^2 - \omega^2 + \omega_{c}\omega}.
\label{eq:09_65}
\eeq
...
\beq
n_L^2 = 1 + \frac{\omega_p^2}{\omega_0^2 -\omega^2 - \omega_{c}\omega} \qquad \mathrm{in}
\qquad 
n_D^2 = 1 + \frac{\omega_p^2}{\omega_0^2 -\omega^2 + \omega_{c}\omega}.
\label{eq:09_66}
\eeq
...
središču kroženja in za drugo stran od njega, torej povzroči, da sta lomna količnika
...
\eeq
Tako smo na preprostem modelu pokazali približno linearno odvisnost kota zasuka od zunanjega 
magnetnega polja. 
