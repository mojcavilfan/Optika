...
\def\svgwidth{130truemm} 
\input{slike/04_vlakno.pdf_tex}
\caption{Akrilni valj deluje kot optično vlakno, po katerem potuje svetloba.}
...
Prepuščeno valovanje v drugi snovi (snovi z manjšim lomnim količnikom) 
...
\centering
\def\svgwidth{140truemm} 
\includegraphics{slike/04_evanescentno.jpg}
%\input{slike/04_simulacija.pdf_tex}
\caption{NOVA SLIKA! Električna poljska jakost pri navadnem (a) in totalnem (b) odboju.
...
\begin{array}{c}
0\\
E_{0t} e^{ik_xx-i\omega t}e^{-\varkappa z}\\
0\\
\end{array}
\right] = 
...
\frac{\partial \mathbf{H}}{\partial t} = \mu_0 i \omega \mathbf{H}
 \label{eq:04_67}
\eeq
izpeljemo vektor jakosti magnetnega polja:
\beq
\mathbf{H} = \frac{-iE_{0t}}{\mu_0 \omega} \left|
\begin{array}{ccc}
...
\beq
\mathbf{H} =
\frac{-iE_{0t}}{\mu_0 \omega} 
\left[
\begin{array}{c}
-\varkappa  e^{ik_xx-i\omega t-\varkappa z}\\
0\\
ik_x e^{ik_xx-i\omega t-\varkappa z}\\
\end{array}
\right] = 
\frac{E_{0t}}{\mu_0 \omega} 
...
\beq
\mathbf{S} = \mathbf{E}\times \mathbf{H}^* = \frac{E_{0t}^2 e^{2\varkappa z }}{\mu_0 \omega} 
\left[
\begin{array}{c}
0\\
1\\
...
\right]
 = 
\frac{E_{0t} e^{-2\varkappa z }}{\mu_0 \omega}
\left[
\begin{array}{c}
k_x\\
0\\
...
\begin{figure}[ht]
\centering
%\includegraphics[width=10truecm]{slike/04_TunelFoto.jpg}
