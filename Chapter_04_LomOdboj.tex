...
\includegraphics[width=5truecm]{slike/04_photos_zmaj1.jpg}
...
\begin{figure}[!h]
\centering
\includegraphics[width=14truecm]{slike/Mavrica.jpeg}
...
kotih je odbojnost enaka 1. Prepustnost je navečja pri Brewstrovem kotu, nato pa strmo
...
po katerih svetloba potuje na zalo dolge razdalje. Optična vlakna so nepogrešljiva
...
\def\svgwidth{130truemm} 
\input{slike/04_vlakno.pdf_tex}
\caption{NOVA SLIKA! Akrilni valj deluje kot optično vlakno, po katerem potuje svetloba.}
...
Iz enačb~(\ref{eq:ev1} in \ref{eq:ev2}) je jasno vidno, da valovanje pri totalnem
odboju potuje vzporedno z osjo $x$. V smeri osi $z$ je valovanje na vpadni 
strani meje interferenčno modulirano, na nasprotni strani meje pa eksponentno
pojemajoče (glej sliko~\ref{fig:04_simulacija}).
\begin{figure}[ht]
\centering
\includegraphics[width=140truemm]{slike/04_sim_lom.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=10truecm]{slike/04_FTIR.jpg}
\caption{NOVA SLIKA: Svetloba se ob prehodu iz vode v zrak totalno odbije, zato prstov za kozarcem
ne vidimo (levo). Ko se prsti dotaknejo kozarca, totalnega odboja ni več in prsti postanejo vidni (desno).}
...
H\"anchen (1919--2013).
Ko se linearno polarizirana svetloba totalno odbije, odbiti žarek ne izhaja iz povsem iste točke, 
kot nanjo vpada vpadni žarek. Videti je, kot da se odbije od neke navidezne ravnine, ki leži v snovi
z manjšim lomnim količnikom. Oddaljenost te navidezne odbojne ravnine je kar enaka vdorni globini, 
...
\beq
r = \frac{n_1 \cos \alpha - i n_2 \kappa}{n_1 \cos \alpha + i n_2 \kappa} = 
\frac{\sqrt{n_1^2\cos^2 \alpha + n_2^2 \kappa^2}\,e^{-i\delta/2}}{\sqrt{n_1^2\cos^2 \alpha + n_2^2 \kappa^2}\,e^{+i\delta/2}}
= e^{-i\delta},
\label{eq:04_84}
\eeq
pri čemer je:
...
\beq
\tan \delta_\mathrm{TM} = \left(\frac{n_1}{n_2}\right)^2 \tan \delta_\mathrm{TE}
\label{eq:04_87}
\eeq
in posledično:
\beq
...
Za TE polarizirano valovanje zapišemo amplitudno odbojnost:
\boxeq{eq:04_99}{
...
\begin{figure}[ht]
\centering
%\includegraphics[width=10truecm]{slike/04_KovinaFoto.jpg}
